...
public class ExpressionTree {
    ...
    // Perform a post-order depth first traversal of the tree to evaluate the expression it represents.
    ...
    public void buildFromInfixExpression(String infix) {
        /*
		 * First, we will convert the expression to postfix form
		 * (we could bypass this and build the tree directly)
		 * See http://penguin.ewu.edu/cscd300/Topic/Stack/WikiExtract.html
		 * as an example of the infix -> postfix conversion algorithm
		 * 
		 * Next, we will build the tree from the postfix string
		 * See http://en.wikipedia.org/wiki/Binary_expression_tree
		 * for the algorithm
		 * 
		*/
    ...
    private String infixToPostfix(String infix) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            if (tokens[i].equals("+")
                    || tokens[i].equals("-")
                    || tokens[i].equals("*")
                    || tokens[i].equals("/")) {// operator found, check stack
                if (s.isEmpty())
                    s.push(tokens[i]);
                else {
                    while (true) {
